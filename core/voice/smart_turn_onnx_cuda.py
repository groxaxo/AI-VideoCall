...
class SmartTurnCuda:
    ...
    def __init__(self, cfg: SmartTurnCudaConfig):
        ...
        try:
            ...
            if "CUDAExecutionProvider" in actual_providers:
                logger.info(f"Smart Turn using CUDA on device {cfg.device_id}")
            else:
                logger.warning(
    ...
    def _validate_model_io(self):
        ...
        try:
            ...
            logger.info(
                f"Smart Turn input: name={input_info.name}, shape={input_info.shape}, type={input_info.type}"
            )
            logger.info(
                f"Smart Turn output: name={output_info.name}, shape={output_info.shape}, type={output_info.type}"
            )
    ...
    def is_end_of_turn(self, audio_f32_16k: np.ndarray) -> tuple[bool, float]:
        ...
        try:
            ...
            if len(audio_f32_16k) < min_samples:
                logger.debug(
                    f"Audio too short ({len(audio_f32_16k)} samples), rejecting"
                )
                return False, 0.0
